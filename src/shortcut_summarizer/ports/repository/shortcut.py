...
class TicketRepository(TicketPort):
    def __init__(
            self, api_key: str, api_url: str, get: Callable = requests.get, extract_str: Callable = extract_str, extract_datetime: Callable = extract_datetime, extract_int: Callable = int
    ) -> None:
    ...
    def fetch_tickets_from_project_since(self, team_id: str, since: datetime) -> Iterator[Ticket]:
    ...
    def _get_last_tickets_since_for_team_id(self, since: datetime, team_id: str) -> Iterator[Ticket]:
    ...
    def _parse_tickets(self, data: Dict) -> Iterator[Ticket]:
        yield from Stream(data.get("data", [])).filter(lambda ticket: ticket.get("id")).map(lambda ticket: self._to_ticket(ticket, list(self._get_ticket_comments(ticket.get('id')))))


    def _fetch_tickets_page(self, since: datetime, team_id: str, next_page_token: Optional[str] = None) -> Optional[Dict]:
    ...
    def _get_ticket_comments(self, ticket_id: str) -> Iterator[Comment]:
        ...
        while True:
            if response := self._fetch_comment_page(ticket_id, next_page_token):
                yield from Stream(response.get("data", [])).map(lambda comment: self._to_comment(comment))
                next_page_token = response.get("next")
                if not next_page_token:
                    break
            else:
                _LOGGER.error("Failed to fetch comments for ticket ID: %d", ticket_id)
                break


    def _fetch_comment_page(self,ticket_id:str,  next_page_token: Optional[int] = None) -> Optional[Dict]:
